...
export async function githubVerifyCallback(accessToken: string, refreshToken: string, profile: GithubProfile, done: Done) {
  const id = profile.id;
...
export async function googleVerifyCallback(accessToken: string, refreshToken: string, profile: GoogleProfile, done: VerifyCallback) {
