...
export async function save2FAToken(
	userId: number,
...
export const verify2FASetup = async (request: Request, response: Response): Promise<void> => {
	...
	try {
		...
		response
			.status(200)
			.json(createMessageResponse(request.t('2fa.setup_verified'), 'info', AppStatusCode.TWO_FA_SETUP_SUCCESS));
	} catch (error) {
...
export const recover2FA = async (request: Request, response: Response): Promise<void> => {
	...
	try {
		...
		if (user.status !== UserStatus.Active || !user.enabled2FA) {
			throw new AppError(
				'errors.2fa_recovery_not_initiated',
				{},
				AppStatusCode.TWO_FA_RECOVERY_NOT_INITIATED,
				200
		...
		if (error instanceof AppError) {
			if (error.code === AppStatusCode.USER_NOT_FOUND) {
				...
				response
					...
					.json(
						createMessageResponse(
							request.t('errors.email_2fa_recovery_send_failed'),
							'error',
							AppStatusCode.EMAIL_TWO_FA_RECOVERY_SEND_FAILED
						)
					);
			} else {
				response
		...
		if (error instanceof AppError) {
			if (error.code === AppStatusCode.USER_NOT_FOUND) {
				...
				response
					...
					.json(
						createMessageResponse(
							request.t('errors.2fa_recovery_failed'),
							'error',
							AppStatusCode.TWO_FA_RECOVERY_FAILED
						)
					);
			} else {
				response
