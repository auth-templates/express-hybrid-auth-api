...
describe('POST /2fa/confirm-recover', () => {
	...
	it('should return 204 when 2FA token is valid', async () => {
		...
		expect(emailService.send2FADisabledEmail).toHaveBeenCalledWith({
			t: expect.anything(),
	...
	it('should return 400 when token is invalid', async () => {
		const userId = 10;
		vi.spyOn(VerificationTokensRepository, 'verify2FAToken').mockRejectedValue(
			new AppError('tokens.2fa.invalid', {}, AppStatusCode.TWO_FA_RECOVERY_TOKEN_INVALID, 400)
		);
		...
		expect(response.body).toEqual({
			messages: [
				{
					text: 'The two-step verification recovery link is invalid or no longer available. Please request a new link to recover your account.',
					severity: 'error',
				},
			],
	...
	it('should return 400 when token is invalid', async () => {
		const userId = 10;
		vi.spyOn(VerificationTokensRepository, 'verify2FAToken').mockRejectedValue(
			new AppError('tokens.2fa.expired', {}, AppStatusCode.TWO_FA_RECOVERY_TOKEN_EXPIRED, 400)
		);
		...
		expect(response.body).toEqual({
			messages: [
				{
					text: 'Your two-step verification recovery link has expired. Please start the recovery process again to regain access to your account.',
					severity: 'error',
				},
			],
	...
	it('should return 400 when token is already used', async () => {
		const userId = 10;
		vi.spyOn(VerificationTokensRepository, 'verify2FAToken').mockRejectedValue(
			new AppError('tokens.2fa.already_used', {}, AppStatusCode.TWO_FA_RECOVERY_TOKEN_ALREADY_USED, 400)
		);
		...
		expect(response.body).toEqual({
			messages: [
				{
					text: 'This two-step verification recovery link has already been used. If you still need access, request a new recovery link.',
					severity: 'error',
				},
			],
	...
	it('should return 500 for internal errors', async () => {
		...
		vi.mocked(UserRepository.disable2FA).mockRejectedValue(
			new AppError('errors.internal', {}, AppStatusCode.INTERNAL_SERVER_ERROR, 500)
		);
