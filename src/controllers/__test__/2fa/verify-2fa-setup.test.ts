...
describe('POST /2fa/setup', () => {
	...
	it('should return 200 when with 2FA setup data', async () => {
		...
		expect(response.body).toEqual({
			messages: [{ text: 'Two-factor authentication has been successfully verified.', severity: 'info' }],
			code: AppStatusCode.TWO_FA_SETUP_SUCCESS,
		});
	});
	...
	it('should return 400 if an unexpected error occurs', async () => {
		...
		expect(response.body).toEqual({
			messages: [{ text: 'The 2FA code must be a 6-digit number.', severity: 'error' }],
		});
	});
