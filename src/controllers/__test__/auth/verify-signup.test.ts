...
describe('POST /auth/verify-signup', () => {
	...
	it('should return 204 for a valid token', async () => {
		...
		const response = await request(app).post('/auth/verify-signup').send({ token: 'signup-valid-1' });
	...
	it('should return 400 for a used token', async () => {
		...
		const response = await request(app).post('/auth/verify-signup').send({ token: 'signup-used-1' });
	...
	it('should return 400 for an expired token', async () => {
		...
		const response = await request(app).post('/auth/verify-signup').send({ token: 'signup-expired-1' });
	...
	it('should return 400 for a non-existent token or invalid token', async () => {
		...
		const response = await request(app).post('/auth/verify-signup').send({ token: 'signup-invalid-1' });
