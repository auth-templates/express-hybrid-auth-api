...
describe('POST /signup', () => {
	...
	it('should return 409 if email is already in use', async () => {
		vi.mocked(UserRepository.createUser).mockRejectedValue(
			new AppError('errors.email_address_already_in_use', {}, AppStatusCode.EMAIL_ALREADY_IN_USE, 409)
		);
		...
		expect(response.body).toEqual({
			messages: [
				{ text: 'This email address is already in use. Please use a different one.', severity: 'error' },
			],
	...
	it('should return 400 if password is missing one digit', async () => {
		...
		expect(response.body).toEqual({
			messages: [{ text: 'Password must contain at least one digit', severity: 'error' }],
		});
	});
	...
	it('should return 400 if password is longer than 100 characters', async () => {
		const response = await request(app)
			...
			.send({
				...signupData,
				password:
					'%Supdasdasdasdasdasdasdassssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssdasdasdasdasdasdasdasd123',
			});

		expect(response.status).toBe(400);
		expect(response.body).toEqual({
			messages: [{ text: 'Password must be at most 100 characters', severity: 'error' }],
		});
	});
	...
	it('should return 400 if password is less than 8 characters', async () => {
		...
		expect(response.status).toBe(400);
		expect(response.body).toEqual({
			messages: [{ text: 'Password must be at least 8 characters', severity: 'error' }],
		});
	});
	...
	it('should return 400 if password does no contain at least one special character', async () => {
		...
		expect(response.status).toBe(400);
		expect(response.body).toEqual({
			messages: [{ text: 'Password must contain at least one special character', severity: 'error' }],
		});
	});
	...
	it('should return 400 if first name is missing', async () => {
		...
		expect(response.status).toBe(400);
		expect(response.body).toEqual({
			messages: [{ text: 'First name must be at least 1 character', severity: 'error' }],
		});
	});
	...
	it('should return 400 if first name is longer than 30 characters', async () => {
		...
		expect(response.status).toBe(400);
		expect(response.body).toEqual({
			messages: [{ text: 'First name must be at most 30 characters', severity: 'error' }],
		});
	});
	...
	it('should return 400 if last name is missing', async () => {
		...
		expect(response.body).toStrictEqual({
			messages: [{ text: 'Last name must be at least 1 character', severity: 'error' }],
		});
	});
	...
	it('should return 400 if last name is longer than 30 characters', async () => {
		...
		expect(response.status).toBe(400);
		expect(response.body).toStrictEqual({
			messages: [{ text: 'Last name must be at most 30 characters', severity: 'error' }],
		});
	});
	...
	it('should return 400 if role is invalid', async () => {
		...
		expect(response.body).toStrictEqual({
			messages: [
				{
					text: 'Invalid type: Expected ("employee" | "manager" | "admin") but received "ds"',
					severity: 'error',
				},
			],
		});
	...
	it('should return 400 if terms and conditions is not accepted', async () => {
		...
		expect(response.status).toBe(400);
		expect(response.body).toStrictEqual({
			messages: [{ text: 'You must accept the terms and conditions.', severity: 'error' }],
		});
	});
});
