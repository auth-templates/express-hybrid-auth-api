...
describe('POST /auth/refresh', () => {
	...
	it('should 403 when no refresh_token cookie', async () => {
		...
		expect(response.body).toEqual({
			messages: [{ text: 'An unexpected error occurred. Please retry to log in.', severity: 'error' }],
			code: AppStatusCode.REFRESH_TOKEN_INVALID,
		});
	});
	...
	it('should 403 when refresh_token does not match stored', async () => {
		...
		const response = await agent
			.post('/auth/refresh')
			.set('Cookie', 'refresh_token=bad-token; connect.sid=session-id');
	...
	it('should handle pending 2FA true state correctly', async () => {
		...
		const response = await agent
			.post('/auth/refresh')
			.set('Cookie', 'refresh_token=good-token; connect.sid=session-id');
		expect(response.status).toBe(204);
