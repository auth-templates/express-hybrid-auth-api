...
describe('authenticate middleware', () => {
	...
	it('should return 401 if session is missing', () => {
		...
		expect(res.json).toHaveBeenCalledWith({
			messages: [{ text: 'Session invalid or expired', severity: 'error' }],
			code: AppStatusCode.SESSION_INVALID_OR_EXPIRED,
		});
	...
	it('should return 401 if access token is missing', () => {
		...
		expect(res.status).toHaveBeenCalledWith(401);
		expect(res.json).toHaveBeenCalledWith({
			messages: [{ text: 'Access token missing', severity: 'error' }],
			code: AppStatusCode.ACCESS_TOKEN_MISSING,
		});
		expect(next).not.toHaveBeenCalled();
	});
	...
	it('should return 401 if token is expired', () => {
		...
		expect(res.status).toHaveBeenCalledWith(401);
		expect(res.json).toHaveBeenCalledWith({
			messages: [{ text: 'Access token expired', severity: 'error' }],
			code: AppStatusCode.ACCESS_TOKEN_EXPIRED,
		});
	});
	...
	it('should return 403 if token is invalid', () => {
		...
		expect(res.json).toHaveBeenCalledWith({
			messages: [{ text: 'Invalid access token', severity: 'error' }],
			code: AppStatusCode.ACCESS_TOKEN_INVALID,
		});
	});
